...
//transaksikan barang yang masuk
if(isset($_POST['masuk'])){
	...
	$brg_masuk = $db->prepare("INSERT INTO far_masuk SET tgl_masuk = ?, id_barang = ?, jml_masuk = ?");
	...
	//cek data persediaan
	$cek = $db->prepare("SELECT COUNT(*) far_stock WHERE id_barang = ?");
	...
	if($cek->fetchColumn() == 1){
		...
		$masuk = $db->prepare("INSERT INTO far_stock SET ");
	}
			...
			<div class="box-body">
				<form method="post" action="" class="form-horizontal">
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input id="tanggal" type="text" name="tgl" class="form-control" required />
						</div>
                    </div>
					<div class="form-group">
						...
						<div class="row">						
							<div class="col-xs-3">
								<?php if(empty($id)){
										echo "<input type=\"text\" name=\"id_barang\" class=\"form-control\" readonly />";
									}else{
										echo "<input type=\"text\" name=\"id_barang\" class=\"form-control\" value=".$id." readonly />";
									}
								?>
							</div>
							<div class="col-xs-2" style="padding:0px">
								<a href="#" class="btn btn-success btn-sm" data-toggle="modal" data-target="#AmbilBarang"><i class="fa fa-search fa-fw"></i>Cari</a>
							</div>
						</div>
                    </div>
					<div class="form-group">
						...
						<div class="col-sm-5">
							<?php if(empty($nama)){
								echo "<input type=\"text\" name=\"nama_barang\" class=\"form-control\" readonly />";
							}else{
								echo "<input type=\"text\" name=\"nama_barang\" class=\"form-control\" value=".$nama." readonly />";
							}
							?>
						</div>
                    </div>
